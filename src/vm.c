...
*/
int getpgtable(struct pt_entry *entries, int num) { // Check if entries is null
  if (entries == 0){
    return -1;
  }
  pte_t *pte;
  ...
  while (numFilled < num && pagesSearched < totalPages) {
    ...
    // make sure they pages are actually valid PTE_P or PTE_E must be 1
